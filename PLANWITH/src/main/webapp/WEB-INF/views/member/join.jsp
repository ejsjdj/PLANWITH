...
<style>
	...
	#joinForm .group:nth-of-type(1) p{	/* 아이디 그룹 */
		display: flex;
		align-items: center;
	...
	#joinForm .group:nth-of-type(1) p span:nth-of-type(1) {
	    display: block;
	    width: 100%; /* 가로 길이를 100%로 설정하여 input이 span 너비에 맞춰서 늘어남 */
	    margin-right: 0.8rem;
	}
...
<form id="joinForm" method="POST" action="${cpath}/member/join">
	...
	<div class="group">
		...
		<p>
			<span>
				<input id="join-userid" class="input" type="text" name="userid" placeholder="아이디" autocomplete="off" required autofocus>
			</span>
			<span>	
				<input class="submit" type="button" id="idCheck" value="확인">	
			</span>
		</p>
	</div>
	...
	<div class="group">
		...
		<p><input id="join-userpw" class="input" type="password" name="userpw" placeholder="비밀번호" required></p>
	</div>
	
	<div class="group">
		<label for="join-nickname" class="small-grey">닉네임</label>
	<p><input id="join-nickname" class="input" type="text" name="nickname" placeholder="닉네임" required></p>
	</div>
	
	<div class="group">
		<label for="join-email" class="small-grey">이메일</label>
	<p><input id="join-email" class="input" type="email" name="email" placeholder="이메일" required></p>
	</div>
	
	<div class="group">
		<label for="join-address" class="small-grey">주소</label>
	<p><input id="join-address" class="input" type="text" name="address" placeholder="주소를 검색하세요" autocomplete="off" required></p>
	</div>
...
// 	    document.getElementById('agreement-content').innerHTML = termsSection
	...
	async function loadCaptchaHandler() {
		...
		tag += '<input class="input" type="text" name="captcha" placeholder="그림에 나타난 글자를 입력하세요" required>'
		tag += '</fieldset>'
