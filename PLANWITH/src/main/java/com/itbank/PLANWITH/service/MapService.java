...
public class MapService {
	...
	public List<NaverBlogDTO> getBlogSearch(String title) {
		try {
			...
			// 테스트용 출력문 연결상태 및 값에 문제가 있을때 사용
//			System.out.println("getBlogSearch 함수 실행");
//			System.out.println(blogSearchResponse);
	...
	public List<NaverImageDTO> getImageSearch(String title) {
		try {
			...
			// 테스트용 출력문 연결상태 및 값에 문제가 있을때 사용
//			System.out.println("getImageSearch 함수 실행");
//			System.out.println(imageSearchResponse);
...
//	// place의 insert 를 수행하는 service 함수
//	public int insertPlace(ScheduleDTO dto) {
//		
//		String name = dto.getTitle();
//		String category = "일반";
//		String city = dto.getAddress().split(" ")[0];
//		int index = dto.getAddress().indexOf(' ');
//		String address = dto.getAddress().substring(index);
//		
//		PlaceDTO placeDTO = new PlaceDTO(name, category, city, address);
//		
//		int result = mapDAO.selectPlace(placeDTO); // 이미 있는지 확인
//		if (result == 0) { // 없으면 인서트를 실행
//			mapDAO.insert(placeDTO);
//		} 
//		return mapDAO.searchId(placeDTO); // 인서트된 값의 id를 불러와 리턴
//	}
